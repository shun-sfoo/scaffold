...
struct SpinBarrier {
  explicit SpinBarrier(std::size_t n) noexcept
      : m_top_waiting((std::uint32_t)n - 1), m_num_waiting(0), m_sync_flip(0) {}
  ...
  bool arrive_and_wait() noexcept {
    ...
    if (m_num_waiting.fetch_add(1, std::memory_order_relaxed) == m_top_waiting) {
      ...
      do {
        if (m_sync_flip.load(std::memory_order_acquire) != old_flip) return false;
        if (m_sync_flip.load(std::memory_order_acquire) != old_flip) return false;
        if (m_sync_flip.load(std::memory_order_acquire) != old_flip) return false;
        if (m_sync_flip.load(std::memory_order_acquire) != old_flip) return false;
      } while (--retries);
...
struct MTTest {
 ...
 private:
  ...
  struct HasSetup<TestClass, std::void_t<decltype(std::declval<TestClass>().setup())>> : std::true_type {};

  template <class TestClass, class = void>
  struct HasTeardown : std::false_type {};

  template <class TestClass>
  struct HasTeardown<TestClass, std::void_t<decltype(std::declval<TestClass>().teardown())>> : std::true_type {};
  ...
  struct CountEntries<TestClass, Begin, std::void_t<decltype(std::declval<TestClass>().entry(MTIndex<Begin>()))>>
  ...
  struct MTTestImpl final : MTTest {
   ...
   private:
    ...
    void setupThreadLocalImpl(std::size_t index, std::index_sequence<Is...>) noexcept {
      testClass = &m_testClass;
      testFunc = nullptr;
      // 带 break 的静态 for 循环，第二步是通过 ... 搏杀暴徒
      (void)((Is == index ? ((testFunc = [] { static_cast<TestClass *>(testClass)->entry(MTIndex<Is>()); }), true)
                          : false) ||
             ...);
    }
    ...
    void setupThreadLocal(std::size_t index) noexcept override {
      ...
      setupThreadLocalImpl(index, std::make_index_sequence<CountEntries<TestClass>::value>());
    }
  ...
  static std::string resultToString(Result result) { return std::to_string(result); };
  ...
  struct MTTestPool final {
    MTTestPool(std::unique_ptr<MTTest> testData, std::size_t repeats)
      ...
      for (std::size_t i = 0; i < m_threads.size(); ++i) {
        m_threads[i] = std::thread(&MTTestPool::testThread, this, i, i % maxCores);
      }
      m_statThread = std::thread(&MTTestPool::statisticThread, this, m_threads.size() % maxCores);
    }
