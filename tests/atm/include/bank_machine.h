 ...
 public:
  ...
  void run() {
    try {
      for (;;) {
        incoming.wait()
            .handle<verify_pin, std::function<void(verify_pin const& msg)>, messaging::dispatcher>(
            ...
            .handle<get_balance, std::function<void(get_balance const& msg)>>(
                [&](get_balance const& msg) { msg.atm_queue.send(::balance(balance)); }, "get_balance")
            .handle<withdrawal_processed, std::function<void(withdrawal_processed const& msg)>>(
                [&](withdrawal_processed const& msg) { std::cout << "withdrawal_processed !!!" << std::endl; },
                "withdrawal_processed")
            .handle<cancel_withdrawal, std::function<void(cancel_withdrawal const& msg)>>(
                [&](cancel_withdrawal const& msg) { std::cout << "cancel withdraw !!!" << std::endl; },
                "cancel_withdrawal");
      }
